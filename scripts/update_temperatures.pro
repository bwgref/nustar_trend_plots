pro update_temperatures

obs = {obsid:'string', object:'string', exposure:'string', obs_start:'string', $
       type:'string', $
       psd0_temp:0., psd1_temp:1., $
       laser0_temp:0., laser1_temp:0., laser0_ext_temp:0., laser1_ext_temp:0.}
...
for s =0, n_elements(socnames) -1 do begin
   ...
   for o =0, n_elements(obsid) -1 do begin
      ...
      IF file_test(outdata+'/'+sname+'/'+sname+'_temp.sav') THEN continue
...
; Compute the mean metrology intensities:
            ...
            obs.laser0_temp = mean(obeb_data.LASER0_TEMP)
            obs.laser1_temp = mean(obeb_data.LASER1_TEMP)
            obs.laser0_ext_temp = mean(obeb_data.L0_EXTHTR_TEMP)
            obs.laser1_ext_temp = mean(obeb_data.L1_EXTHTR_TEMP)

            ceb_file = file_search(socdir+'/hk/', '*ceb*')
            ceb_data = mrdfits(ceb_file, 1, /silent)
            obs.psd0_temp = mean(ceb_data.psd0temp)
            obs.psd1_temp = mean(ceb_data.psd1temp)
            good = 1
         endif 
      ENDIF
   ENDIF
   ...
   save, obs, file = outdir+'/'+obs.obsid+'_temp.sav'
endfor
